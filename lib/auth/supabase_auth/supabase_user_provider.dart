...
class ToothyCOSupabaseUser extends BaseAuthUser {
  ToothyCOSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> toothyCOSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ToothyCOSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
